...
test.describe('CU-FULL: User journey CRUD + audit', () => {
  test('login -> create persona -> consult -> view audit log', async ({ page }) => {
    ...
    await page.selectOption('select[name="tipo_documento"]', { label: 'Cédula' });
    ...
    await page.fill('input[name="correo_electronico"]', `ana.${Date.now()}@example.com`);
    ...
    await expect(page.locator('.alert-success, .toast-success, [role="alert"]')).toBeVisible({ timeout: 10000 });
    ...
    // Search by document
    await page.fill('input[name="numero_documento"], input[placeholder*="documento"]', DOC);
    ...
    const logsLinks = page.locator('a:has-text("Logs"), a:has-text("Auditoría"), a:has-text("Ver Logs")');
    ...
    const maybeRow = page.locator(`text=CREATE`, { has: page.locator(`text=${DOC}`) });
