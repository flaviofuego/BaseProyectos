  ...
  setupEventListeners() {
    ...
    document.addEventListener("click", (e) => {
      if (
        ...
        const button = e.target.matches(".theme-toggle")
          ? e.target
          : e.target.closest(".theme-toggle");
    ...
    const handler = (e) => {
      ...
      const code = e.code || "";
      const keyCode = e.keyCode || e.which || 0; // 84 for 'T'
      const isT = key === "t" || code === "KeyT" || keyCode === 84;
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && isT) {
        e.preventDefault();
        this.toggleTheme();
