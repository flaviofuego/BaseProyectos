...
class ContentManager {
  ...
  setupTableHandlers() {
    ...
    document.addEventListener("click", (e) => {
      if (e.target.matches('button[type="submit"], .btn-search')) {
        ...
        if (form) {
          const table = form.parentElement.querySelector(
  ...
  generateLoaderId() {
    return (
      "loader_" + Date.now() + "_" + Math.random().toString(36).substr(2, 9)
    );
  }
...
class NotificationManager {
  ...
  show(message, type = "info", duration = 5000, options = {}) {
    ...
    // Icono y mensaje (sin título para mayor compactness)
    const icon = this.getIcon(type);
    notification.innerHTML = `
            <div style="display: flex; align-items: center; gap: 0.75rem;">
                <span style="font-size: 1.25em; flex-shrink: 0; opacity: 1;">${icon}</span>
                <div style="flex: 1; font-size: 0.95em; font-weight: 500; opacity: 1;">
                    ${message}
                </div>
                <button onclick="this.parentElement.parentElement.remove()" 
                        style="background: none; border: none; color: inherit; opacity: 0.7; cursor: pointer; padding: 0; margin-left: 0.5rem; font-size: 1.4em; line-height: 1; transition: opacity 0.2s;"
