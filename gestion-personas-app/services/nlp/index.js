      ...
      points: [
        {
          ...
          payload: {
            ...
            nombre_completo: `${persona.primer_nombre} ${
...
function classifyQueryFallback(query) {
  ...
  if (
    ...
    lowerQuery.includes("configuración")
  ) {
  ...
  if (
    lowerQuery.includes("más viejo") ||
    lowerQuery.includes("más vieja") ||
    lowerQuery.includes("mayor edad")
  ) {
    return { intent: "oldest", parameters: {} };
  }

  if (
    lowerQuery.includes("cuántas") ||
    lowerQuery.includes("cuantas") ||
    lowerQuery.includes("contar")
  ) {
  ...
  if (
    ...
    lowerQuery.includes("estadisticas") ||
...
Respuesta JSON:`;
      ...
      console.log(
        "Usando clasificación local (Gemini no disponible):",
        geminiError.message
      );
    ...
    switch (intent.intent) {
      case "security_blocked":
        return {
          answer:
            "Lo siento, no puedo proporcionar información sobre configuraciones del sistema o variables de entorno por razones de seguridad. Solo puedo ayudarte con consultas sobre las personas registradas en la base de datos.",
          data: null,
          intent: intent,
        };
      ...
      case "search":
        ...
        if (searchResults.length > 0) {
          ...
          return {
            ...
            data: result.rows,
            intent: intent,
            similarity_scores: searchResults.map((r) => ({
              id: r.id,
              score: r.score,
            })),
          };
      ...
      case "stats":
        ...
        return {
          answer: `En el sistema hay ${statsData.total} personas registradas. 
                   ${statsData.masculino} son masculinos, ${
            statsData.femenino
          } son femeninos, 
                   y ${statsData.no_binario} se identifican como no binario. 
                   La edad promedio es de ${Math.round(
                     statsData.edad_promedio
                   )} años.`,
          data: statsData,
          intent: intent,
        };
      ...
      case "name_starts_with":
        ...
        if (result.rows.length > 0) {
          return {
            answer: `Encontré ${
              result.rows.length
            } persona(s) cuyo ${field} empieza con "${letter}": ${result.rows
              .map(
                (p) =>
      ...
      case "direct_search":
        let searchQuery =
          "SELECT *, EXTRACT(YEAR FROM AGE(fecha_nacimiento)) as edad FROM personas WHERE 1=1";
        const searchParams = [];
        ...
        if (result.rows.length > 0) {
          return {
            answer: `Encontré ${
              result.rows.length
            } persona(s) que coinciden con los criterios: ${result.rows
              .map(
                (p) =>
                  `${p.primer_nombre} ${p.segundo_nombre || ""} ${p.apellidos}`
              )
          ...
          return {
            answer:
              "No encontré personas que coincidan con los criterios especificados.",
            data: [],
            intent: intent,
          };
        }
...
app.listen(PORT, () => {
  ...
  const serviceConfig = {
    ...
    metadata: {
      version: "1.0.0",
      description:
        "Natural Language Processing service using Google Gemini and vector search",
      maintainer: "nlp-team",
