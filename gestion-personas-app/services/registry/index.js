...
app.post("/register", (req, res) => {
  try {
    ...
    console.log(
      `✅ Service registered: ${name} (${serviceId}) at ${service.url}`
    );
...
app.get("/health", (req, res) => {
  res.json({
    ...
    registry: {
      ...
      healthyServices: Array.from(services.values()).filter((s) =>
        s.isHealthy()
      ).length,
    },
  });
  ...
  try {
    ...
    switch (strategy) {
      ...
      default:
        ...
        selectedService = healthyServices.sort(
...
if (process.env.NODE_ENV !== "test") {
  app.listen(PORT, () => {
    ...
    console.log(
      `🔍 Discovery endpoint: http://localhost:${PORT}/discover/{serviceName}`
    );
