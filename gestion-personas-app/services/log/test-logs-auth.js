...
async function testLogs() {
  try {
    ...
    const loginResponse = await axios.post('http://gateway:8001/api/auth/login', {
      username: 'admin',
      password: 'admin123'
    });
    ...
    const logsResponse = await axios.get('http://gateway:8001/api/logs/search', {
      headers: {
    ...
    const statsResponse = await axios.get('http://gateway:8001/api/logs/stats', {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
